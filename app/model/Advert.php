...
class Advert extends Base {
    ...
    function __construct(){
        $this->table = $this->tableInit('advert');
    }
    /**
     * 获取列表
     * @param string $type
    ...
    public function getListPage($type,$condition=[],$order=[],$page=1,$pageSize=15){
        switch ($type){
            case 'list':
                $field = ['id','name','status','utime','ctime','url','type','price','publish_time'];
                break;
        }
    ...
    public function getDetail($type,$condition){
        switch ($type){
            case 'edit':
                $field = ['id','name','status','desc','content','cover','url','type','price','publish_time'];
                break;
        }
