...
class Train extends Base {
    ...
    function __construct(){
        $this->table = $this->tableInit('train');
    }
    /**
     * 获取列表
     * @param string $type
    ...
    public function getList($type,$condition=[],$order=[],$limit=100){
        switch ($type){
            case 'cache':
                $field = ['id','title','author','utime','ctime','status'];
                break;
        }
    ...
    public function getListPage($type,$condition=[],$order=[],$page=1,$pageSize=15){
        switch ($type){
            case 'list':
                $field = ['id','title','author','utime','ctime'];
                break;
        }
    ...
    public function getDetail($type,$condition){
        switch ($type){
            case 'edit':
                $field = ['id','title','author','cover','utime','desc','content'];
                break;
        }
