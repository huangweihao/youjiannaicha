...
class Schedule extends Base {
    ...
    function __construct(){
        $this->table = $this->tableInit('schedule');
    }
    /**
     * 获取列表
     * @param string $type
    ...
    public function getListPage($type,$condition=[],$order=[],$page=1,$pageSize=15){
        switch ($type){
            case 'list':
                $field = ['id','activity_id','user_id','schedule_begin','schedule_end','utime','ctime'];
                break;
        }
    ...
    public function getDetail($type,$condition){
        switch ($type){
            case 'edit':
                $field = ['id','title','schedule_begin','schedule_end','activity_id','user_id'];
                break;
        }
