...
class ThirdpartyUser extends Base {
    ...
    public function list(){
        ...
        $order = ['id'=>'desc'];
        $result = $this->ThirdpartyUserIns->getListPage('list',$condition,$order,$this->page,$this->pageSize);
//        dump($result);die;
        if($result['total'] > 0){
     ...
     */
    public function e(){
        $result = (new CommonWork($this,$this->ThirdpartyUserIns))->checkEdit();
//        dump($result);die;
        if(!empty($result)){
    ...
    private function doWork($type){
        ...
        switch ($type){
            case 'insert':
                ...
                $result = $this->ThirdpartyUserIns->inData($data);
                if(!empty($result)){
                    $params['key'] = $result;
                }
                break;
            case 'edit':
                if(empty($params['key'])){
                    ...
                    $result = $this->ThirdpartyUserIns->upData($condition,$data);
                }
                break;
        }
        if(isset($result) ){
     ...
     */
    public function dou(){
        return (new CommonWork($this,$this->ThirdpartyUserIns))->doUpdate(true);
    }
    ...
    public function setCache(){
        $data = [];
        $result = $this->ThirdpartyUserIns->getList('cache');
        if(!empty($result)){
     ...
     */
    public function dod(){
        return (new CommonWork($this,$this->ThirdpartyUserIns))->doDelete(true);
    }
}