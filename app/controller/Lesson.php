...
class Lesson extends Base {
    protected $lessonIns = null;
    protected $mapIns = null;
    protected $sword = null;
    public function __construct(){
        parent::__construct();
        $this->lessonIns = (new LessonModel());
    ...
    public function list(){
        ...
        $order = ['id'=>'desc'];
        $result = $this->lessonIns->getListPage('list',$condition,$order,$this->page,$this->pageSize);
//        dump($result);die;
        if($result['total'] > 0){
     ...
     */
    public function e(){
        $result = (new CommonWork($this,$this->lessonIns))->checkEdit();
//        dump($result);die;
        if(!empty($result)){
    ...
    private function doWork($type){
        ...
        switch ($type){
            case 'insert':
                ...
                $result = $this->lessonIns->inData($data);
                if(!empty($result)){
                    $params['key'] = $result;
                }
                break;
            case 'edit':
                if(empty($params['key'])){
                    ...
                    $result = $this->lessonIns->upData($condition,$data);
                }
                break;
        }
        if(isset($result) ){
     ...
     */
    public function dou(){
        return (new CommonWork($this,$this->lessonIns))->doUpdate(true);
    }
    ...
    public function setCache(){
        $data = [];
        $result = $this->lessonIns->getList('cache');
        if(!empty($result)){
     ...
     */
    public function dod(){
        return (new CommonWork($this,$this->lessonIns))->doDelete(true);
    }
}