...
class Message extends Base {
    protected $MessageIns = null;
    protected $mapIns = null;
    protected $sword = null;
    public function __construct(){
        parent::__construct();
        $this->MessageIns = (new MessageModel());
    ...
    public function list(){
        ...
        $condition = [];
        if(!empty($this->sword)){
            $condition['content'] = ['rule'=>'like','val'=>$this->sword];
        }
        $order = ['id'=>'desc'];
        $result = $this->MessageIns->getListPage('list',$condition,$order,$this->page,$this->pageSize);
        if($result['total'] > 0){
     ...
     */
    public function e(){
        $result = (new CommonWork($this,$this->MessageIns))->checkEdit();
//        dump($result);die;
        if(!empty($result)){
    ...
    private function doWork($type){
        ...
        switch ($type){
            case 'insert':
                ...
                $result = $this->MessageIns->inData($data);
                if(!empty($result)){
                    $params['key'] = $result;
                }
                break;
            case 'edit':
                if(empty($params['key'])){
                    ...
                    $result = $this->MessageIns->upData($condition,$data);
                }
                break;
        }
        if(isset($result) ){
     ...
     */
    public function dou(){
        return (new CommonWork($this,$this->MessageIns))->doUpdate(true);
    }
    ...
    public function setCache(){
        $data = [];
        $result = $this->MessageIns->getList('cache');
        if(!empty($result)){
     ...
     */
    public function dod(){
        return (new CommonWork($this,$this->MessageIns))->doDelete(true);
    }
}