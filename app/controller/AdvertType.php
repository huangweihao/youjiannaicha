...
class AdvertType extends Base {
    protected $advertTypeIns = null;
    protected $modelMapIns = null;
    protected $sword = null;
    public function __construct(){
        parent::__construct();
        $this->advertTypeIns = (new AdvertTypeModel());
    ...
    public function list(){
        ...
        $order = ['id'=>'desc'];
        $result = $this->advertTypeIns->getListPage('list',$condition,$order,$this->page,$this->pageSize);
        if($result['total'] > 0){
     ...
     */
    public function e(){
        $result = (new CommonWork($this,$this->advertTypeIns))->checkEdit();
        if(!empty($result)){
    ...
    private function doWork($type){
        ...
        switch ($type){
            case 'insert':
                ...
                $result = $this->advertTypeIns->inData($data);
                if(!empty($result)){
                    $params['key'] = $result;
                }
                break;
            case 'edit':
                if(empty($params['key'])){
                    ...
                    $result = $this->advertTypeIns->upData($condition,$data);
                }
                break;
        }
        if(isset($result) ){
            ...
            if($code == 200){
                ...
                $this->doEasyRedis('set', ['key' => 'advert_type_'.$params['key'], 'val'=>$params['title']]);
            }
     ...
     */
    public function dou(){
        return (new CommonWork($this,$this->advertTypeIns))->doUpdate(false);
    }
     ...
     */
    public function dod(){
        return (new CommonWork($this,$this->advertTypeIns))->doDelete(false);
    }
}