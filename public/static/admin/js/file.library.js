;(function ($, window, document, undefined) {
    ...
    FileLibrary.prototype = {
        ...
        deleteFilesEvent: function () {
            ...
            _this.$element.on('click', '.file-delete', function () {
                ...
                layer.confirm('确定删除选中的文件吗？', {title: '友情提示'}, function (index) {
                    var load = layer.load();
                    $.post(IMGUP_URL + 'file/deleteFiles', {
                        fileIds: fileIds
                    }, function (result) {
                        layer.close(load);
                        if (result.code == 200) {
        ...
        uploadImagesEvent: function () {
            ...
            uploader.on('uploadSuccess', function (file, response) {
                if (response.code == 202) {
                    ...
                    $list.prepend(template('tpl-file-list-item', [response.result]));
                } else {
        ...
        addGroupEvent: function () {
            ...
            _this.$element.on('click', '.group-add', function () {
                layer.prompt({title: '请输入新分组名称'}, function (value, index) {
                    var load = layer.load();
                    $.post(IMGUP_URL + 'file/addGroup', {
                        group_name: value,
                        group_type: _this.options.type
                    }, function (result) {
                        layer.msg(result.msg);
                        if (result.code == 200) {
                            $groupList.append(template('tpl-group-item', result.result));
                            var $groupSelectList = _this.$element.find('.group-select > .group-list');
                            $groupSelectList.append(
                                '<li>' +
                                '    <a class="move-file-group" data-group-id="' + result.result.group_id + '"' +
                                '       href="javascript:void(0);">' + result.result.group_name + '</a>' +
                                '</li>'
                            );
                        }
                        layer.close(load);
        ...
        deleteGroupEvent: function () {
            ...
            _this.$element.find('.file-group').on('click', '.group-delete', function () {
                ...
                layer.confirm('确定删除该分组吗？', {title: '友情提示'}, function (index) {
                    var load = layer.load();
                    $.post(IMGUP_URL + 'file/deleteGroup', {
                        group_id: group_id
                    }, function (result) {
                        layer.msg(result.msg);
                        if (result.code == 200) {
        ...
        getJsonData: function (params, success) {
            ...
            // 获取文件库列表
            $.getJSON(IMGUP_URL + 'File/fileList', params, function (result) {
                layer.close(loadIndex);
                if (result.code == 200) {
